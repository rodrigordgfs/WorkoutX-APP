...
const WorkoutListItem = ({ workout, isOpen, onToggle }: WorkoutListItemProps) => {
    return (
        <div key={workout.id} className="bg-white dark:bg-zinc-800 rounded-lg shadow-lg overflow-hidden">
            <div className="p-6">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-xl font-semibold text-zinc-900 dark:text-zinc-100">{workout.name}</h3>
                        </div>
                    </div>
                    <button 
                    ...
                    >
                        {isOpen ? <ChevronUp size={24} className="text-zinc-600 dark:text-zinc-300" /> : <ChevronDown size={24} className="text-zinc-600 dark:text-zinc-300" />}
                    </button>
                </div>
            </div>
                ...
                <div className="border-t border-zinc-100 dark:border-zinc-700">
                    <div className="grid divide-y divide-zinc-100 dark:divide-zinc-700">
                        {workout.exercises.map((exercise) => (
                            <div key={exercise.id} className="p-4 hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-colors">
                                <div className="flex items-center justify-between">
                                    <div className="flex-1">
                                        <h4 className="font-medium text-zinc-900 dark:text-zinc-100">{exercise.name}</h4>
                                        <div className="mt-1 flex items-center gap-4 text-sm text-zinc-600 dark:text-zinc-400">
                                            ...
                                            {exercise.weight && (
                                                <div className="flex items-center gap-1">
                                                    <Weight size={16} className="text-blue-500 dark:text-blue-400" />
