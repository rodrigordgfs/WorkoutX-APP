                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                          <div>
                            ...
                            <input
                              ...
                              type="number"
                              value={exercise.sets}
                              onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateExerciseConfig(exercise.id, 'sets', parseInt(e.target.value) || 0)}
                              min="1"
                              className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                            />
                          </div>
                          <div>
                            ...
                            <input
                              id={`reps-${exercise.id}`}
                              value={exercise.reps}
                              onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateExerciseConfig(exercise.id, 'reps', e.target.value)}
                              placeholder="8-12"
                              className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                            />
                          </div>
                          <div>
                            <label htmlFor={`weight-${exercise.id}`} className="block text-xs sm:text-sm font-medium mb-1 sm:mb-2">Peso</label>
                            <input
                              id={`weight-${exercise.id}`}
                              value={exercise.weight}
                              onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateExerciseConfig(exercise.id, 'weight', e.target.value)}
                              placeholder="0kg"
                              className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                            />
                          </div>
                          <div>
                            <label htmlFor={`rest-${exercise.id}`} className="block text-xs sm:text-sm font-medium mb-1 sm:mb-2">Descanso</label>
                            <input
                              id={`rest-${exercise.id}`}
                              value={exercise.rest}
                              onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateExerciseConfig(exercise.id, 'rest', e.target.value)}
                              placeholder="60s"
                              className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                            />
                          </div>
